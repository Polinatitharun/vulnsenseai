...
class PromptGenerate(APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            self.append_log(
                f"Processing prompt generation request for user: {request.user.username} with data: {request.data}"
            )
            ...
            if serializer.is_valid():
                ...
                # Save original request first
                instance = serializer.save(performed_by=request.user)

                # Read required fields
                input_prompt = serializer.validated_data['input_prompt']
                prompt_type = serializer.validated_data.get('prompt_type', None)   # 🔥 NEW: read prompt_type

                # 🔥 NEW: Combine prompt_type + input_prompt into a single string for LLM
                combined_prompt = (
                    f"Prompt Type: {prompt_type}\nUser Prompt: {input_prompt}"
                    if prompt_type else input_prompt
                )

                self.append_log(f"Combined LLM input: {combined_prompt[:60]}...")   # 🔥 NEW LOG

                # 🔥 UPDATED: send the combined prompt to LLM
                generated_response = self.prompt_santization(combined_prompt)
                ...
                self.append_log(
                    f"Prompt generation completed and saved for instance ID: {instance.id}"
                )
    ...
    def post(self, request):
        ...
        Output in the required JSON format only."""
            ...
            def invoke_model(model, prompt):
                ...
                print("prompts generation for load test started")
                return res
...
from rest_framework.response import Response
from rest_framework import status
from .models import Target, SecurityScanReport, Testing
from .serializers import SecurityScanSerializer, TestingSerializer
from . import scans
import threading
...
class SecurityScanView(APIView):
    """Security Scan API - runs multiple vulnerability scans concurrently"""
    ...
    def post(self, request):
        try:
            target_id = request.data.get('target')
            if not target_id:
                return Response("target ID is required", status=status.HTTP_400_BAD_REQUEST)
            ...
            # ---- Prepare vulnerability scan functions ----
            vulnerability_scans = [
            ...
            # ---- Dictionary to aggregate scan results and reports ----
            aggregated_results = {}
            scan_reports = {}  # To store SecurityScanReport objects for ID lookup

            # ---- Function to process each scan ----
            def process_scan(func):
                try:
                    self.append_log(f"Creating security scan report for vulnerability: {func.__name__}")
                    test_report = SecurityScanReport.objects.create(
                        target=target,
                        test=security_test,
                        vulnerability=func.__name__
                    )
                    self.append_log(f"Starting scan: {func.__name__} for test ID: {test_report.id}")

                    func_response = func(target.model_name)
                    if not isinstance(func_response, dict):
                        func_response = {}

                    # ---- Truncate large responses for logging ----
                    truncated_response = str(func_response)
                    if len(truncated_response) > 200:
                        truncated_response = truncated_response[:200] + "...[truncated]"

                    self.append_log(f"Scan completed: {func.__name__} with response: {truncated_response}")

                    # ---- Save scan report using serializer ----
                    test_data = {
                        'prompt_json': func_response.get('prompt', ''),
                        'response_json': func_response.get('response', ''),
                        'result_json': func_response.get('results', '{}'),
                    }
                    serializer = SecurityScanSerializer(test_report, data=test_data, partial=True)
                    if serializer.is_valid():
                        serializer.save()
                        self.append_log(f"Security scan report saved for {func.__name__}, test ID: {test_report.id}")
                    else:
                        self.append_log(f"Invalid serializer data for {func.__name__}: {serializer.errors}")

                    # ---- Add to aggregated results and reports ----
                    aggregated_results[func.__name__] = func_response.get('results', {})
                    scan_reports[func.__name__] = test_report
                    return test_report
                except Exception as scan_e:
                    self.append_log(f"Error in scan {func.__name__}: {str(scan_e)}")
                    logger.error("Error in scan %s: %s", func.__name__, str(scan_e))
                    return None
            ...
            # ---- Create response results with id, status, accuracy_score, and summary ----
            response_results = {}
            for vuln_name, result in aggregated_results.items():
                report = scan_reports.get(vuln_name)
                risk_score = result.get('risk_score', 0)
                response_results[vuln_name] = {
                    ...
                    'accuracy_score': risk_score * 100  # Convert to percentage
                }

            # ---- Update main Testing record with full results ----
            testing_data = {
                'test_status': 'completed',
            ...
            if serializer.is_valid():
                ...
                self.append_log(f"Invalid testing serializer data: {serializer.errors}")

            # ---- Return results to frontend ----
            return Response({
                "message": "Security Scan is completed!",
                "testing_id": security_test.id,
                "results": response_results
            }, status=status.HTTP_201_CREATED)

        except Exception as e:
            logger.error("Error in security scan: %s", str(e))
            self.append_log(f"Error in security scan for target ID: {target_id if target_id else 'Unknown'}: {str(e)}")
            return Response("Error in security scan", status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
import io
class TestID(APIView):
    def post(self,request):
        
        target=request.data.get("target")
        test_type=request.data.get("test_type")
        
        test=Testing.objects.filter(target_id=target,test=test_type,test_status="completed").order_by("start_time")
        # print(test)
        serializer=TestingSerializer(test,many=True)
...
import math
class FuzzGraph(APIView):
    def get(self,request,pk):
        # test_id=request.data.get("test")
        try:
            test=Testing.objects.get(pk=pk)
            result_json=test.results
            # print(result_json)
            overall_accuracy_score=0
            accuracy_scores=[]
            for k,v in result_json.items():
                if 'overa_acc_score' in v:
                    overall_accuracy_score=v.get('overa_acc_score')
                accuracy_scores.append(v.get("accuracy_score"))
            plt.figure(figsize=(12,8))
            plt.plot(accuracy_scores,color='blue',marker='o',linestyle='-',linewidth=2)
            ...
            response['Content_Disposition']=f'inline; filename="Accuarcy Chart_{test.target.model_name}.png"'
            return response
        except Exception as e:
            return Response(str(e),status=status.HTTP_400_BAD_REQUEST)
from reportlab.lib.utils import ImageReader
class FuzzTestReportPDF(APIView):
    def get(self,request,pk):
        # test_id=request.data.get("test")
        try:
            test=Testing.objects.get(pk=pk)
            ...
            for v in result_json.values():
                if isinstance(v,dict) and "overa_acc_score" in v:
                    overall_score=v["overa_acc_score"]
                else:
                    test_cases.append(v)
            # print(test_cases)
            buffer=io.BytesIO()
            ...
            p.drawString(50, y, "Fuzz Test Report")
            y -= 40

    
            if overall_score is not None:
                p.setFont("Helvetica-Bold", 14)
                p.drawString(50, y, f"Overall Accuracy Score: {overall_score}")
                y -= 30
            ...
            fig=go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    'steps':[
                        ...
                        {'range':[41,70],'color':"#FFFF00"},
                        {'range':[71,100],'color':"#008000"}
                    ]
                }
            ))
            # print("figure is created")
            img_bytes=fig.to_image(format="png",width=600,height=400,engine='kaleido')
            # print("figure is rendering")
            img_buffer=io.BytesIO(img_bytes)
            img = ImageReader(img_buffer)

                    
            plt.figure(figsize=(10, 6))
            plt.plot(accuracy_scores, marker='o', linestyle='-', linewidth=2)
            plt.title('Accuracy Scores Line Chart')
            plt.xlabel('Index')
            plt.ylabel('Accuracy')
            plt.grid(True)
            ...
            p.drawImage(img, 50, y - 420, width=500, height=400, preserveAspectRatio=True)
            y -= 440 

            p.drawImage(line_chart, 50, y - 300, width=500, height=250, preserveAspectRatio=True)
            y -= 320

            
            for case in test_cases:
                p.setFont("Helvetica-Bold", 12)
                p.drawString(50, y, f"ID: {case.get('id','')}")
                y -= 15

        
                prompt_para = Paragraph(f"<b>Prompt:</b> {case.get('prompt','')}", normal_style)
                w, h = prompt_para.wrap(width - 100, height)
                if y - h < 50: 
                    p.showPage()
                    y = 800
                prompt_para.drawOn(p, 50, y - h)
                y -= h + 5

            
                response_para = Paragraph(f"<b>Response:</b> {case.get('response','')}", normal_style)
                w, h = response_para.wrap(width - 100, height)
                if y - h < 50:
                    p.showPage()
                    y = 800
                response_para.drawOn(p, 50, y - h)
                y -= h + 10

                
                p.setFont("Helvetica", 10)
                p.drawString(50, y, f"Status: {case.get('status','')}")
                y -= 15
                p.drawString(50, y, f"Summary: {case.get('summary','')}")
                y -= 15
                p.drawString(50, y, f"Accuracy: {case.get('accuracy_score','')}")
                y -= 25
                # print(p)
            # p.showPage()
            p.save()
            buffer.seek(0)
            pdf_name=f"{test.test} {test.id} report.pdf"
            print("pdf is created")
            
            test.results_file.save(pdf_name,ContentFile(buffer.getvalue()))
            test.save()
            #send response
...
class SecurityScanReportPDF(APIView):
    def get(self, request, pk):
        try:
            test=Testing.objects.get(pk=pk)
            scans=SecurityScanReport.objects.filter(test_id=test.id)

            buffer = BytesIO()
            doc = SimpleDocTemplate(buffer, pagesize=A4)
            styles = getSampleStyleSheet()
            elements = []
            print("IO is imported")
    
            elements.append(Paragraph(f"<b>Vulnerability Report for Target</b>", styles['Title']))
            elements.append(Spacer(1, 12))
            
            elements.append(Paragraph(f"<b>Target Name:</b> {test.target.model_name}", styles['Normal']))
            
            elements.append(Spacer(1, 20))
            
            for scan in scans:
                elements.append(Paragraph(f"<b> Vulnerability Name:</b> {scan.vulnerability}", styles['Heading2']))
                elements.append(Spacer(1, 8))
                result=scan.result_json or {}
                # print(result,"this result")
                vulnerability_count=result.get("vulnerability_counts") or {}
                risk_score=result.get('risk_score','N/A')
                # print("risk_score")
                if vulnerability_count:
                    df=pd.DataFrame(
                    {
                        "severity":list(vulnerability_count.keys()),
                        "count":list(vulnerability_count.values())
                    }
                    )
                    print(df)
                    sns.set(style="whitegrid")
                    plt.figure(figsize=(10,8))
                    ax=sns.barplot(data=df,x="severity",y="count",palette="coolwarm",hue=None,legend=False)
                    print(scan.vulnerability)
                    plt.title(f"{scan.vulnerability}")
                    ...
                    # print("image is ready to save")
                    img_buffer=BytesIO()
                    plt.savefig(img_buffer,format="png")
                    # print("image is saves")
                    plt.close()
                    img_buffer.seek(0)
                    # print("image_buffer")
                    chart_img=Image(img_buffer)
                    # print("chart_img",chart_img)
                    chart_img._restrictSize(400,500)
                    elements.append(chart_img)
                    elements.append(Spacer(1, 12))
                else:
                    elements.append(Paragraph("No chart available for this",styles['Heading4']))
                    elements.append(Spacer(1,12))
                elements.append(Paragraph(f"<b>Prompts: </b>", styles['Heading3']))
                i=1
                if scan.prompt_json:
                    for field_label, field_value in scan.prompt_json.items():
                        elements.append(Paragraph(f"<b>Testcase {i}: </b>", styles['Heading4']))
                        i+=1
                        for k,v in field_value.items():
                            formatted_json = json.dumps(v, indent=4)
                            
                            elements.append(Paragraph(f"<b>{k} :</b><font face='Courier'>{formatted_json}</font>", styles['Heading5']))
                    
                    elements.append(Spacer(1, 12))
                else:
                    elements.append(Paragraph("No prompts available for this",styles['Heading4']))
                elements.append(Paragraph(f"<b>Responses: </b>", styles['Heading3']))
                j=1
                if scan.response_json:
                    for field_label,field_value in scan.response_json.items():
                        
                        formatted_json = json.dumps(field_value, indent=4)
                        formatted_json1=formatted_json.replace(r'\n',' ')
                        elements.append(Paragraph(f"<b>Response {j} :</b><font face='Courier'>{formatted_json1}</font>", styles['Heading5']))
                    else:
                        elements.append(Paragraph("No responses available for this",styles['Heading4']))
                    j+=1
                elements.append(Paragraph(f"<b>Results: </b>", styles['Heading3']))
                a=1
                if scan.result_json:
                    for field_label, field_value in scan.result_json.items():
                    
                        print(field_label,field_value)
                        elements.append(Paragraph(f"<b>Result {a}: </b>", styles['Heading4']))
                        a+=1
                        formatted_json = json.dumps(field_value, indent=4)
                                
                        elements.append(Paragraph(f"<b>{field_label} :</b><font face='Courier'>{formatted_json}</font>", styles['Heading5']))
                else:
                    elements.append(Paragraph("No results available for this",styles["Heading4"]))
            ...
            try:
                ...
                response['Content-Disposition'] = f'attachment; filename="target_{test.target.id}_report.pdf"'
                # data={
                #     'response':response,
                #     'analytics':d
                # }
...
class LoadTestGraph(APIView):
    def get(self,request,pk):
        try:
            test=Testing.objects.get(pk=pk)
            overall_score=test.results.get('accuracy_score')
            fig=go.Figure(go.Indicator(
            mode="gauge+number",
...
class LoadTestPDF(APIView):
    def get(self,request,pk):
        try:
            test=Testing.objects.get(pk=pk)
            prompts_list=test.prompt
            # prompts_list1=list(prompts_list[0])
            # print(prompts_list1)
            responses=test.response
            results=test.results
            accuracy_score=int(results.get("accuracy_score"))
            load_score=int(results.get('load_score'))
            buffer = BytesIO()
            doc = SimpleDocTemplate(buffer, pagesize=A4)
            styles = getSampleStyleSheet()
            elements = []
            elements.append(Paragraph(f"<b>Load Test Report for {test.target.model_name}</b>", styles['Title']))
            elements.append(Spacer(1, 12))
            ...
            df=pd.DataFrame(
            {
                "Metrics":["Accuracy score","Load Score"],
                "Scores":[accuracy_score,load_score]
            ...
            sns.set(style="whitegrid")
            plt.figure(figsize=(10,8))
            ax=sns.barplot(data=df,x="Metrics",y="Scores",palette="coolwarm",hue=None,legend=False)
            # print(scan.vulnerability)
            plt.title(f"Load Test Result for {test.target.model_name}")
            plt.xticks(rotation=45,fontsize=10)
            plt.xlabel("Metrics")
            plt.ylabel("Scores")
            # plt.ylim(0,max(df["count"])+2)
            ax.legend(
            [f"Load Score: {load_score} , Accuracy Score: {accuracy_score}"],
            loc="upper right",
            frameon=True,
            fontsize=11,
            facecolor='white',
            ...
            elements.append(Paragraph(f"<b>Prompts: </b>", styles['Heading3']))
            # a=1
            # for i in prompts_list1:
            print(type(prompts_list))
            elements.append(Paragraph(f"{prompts_list}", styles['Normal']))
            # a+=1
            elements.append(Spacer(1, 20))
            elements.append(Paragraph(f"<b>Responses: </b>", styles['Heading3']))
            elements.append(Paragraph(f"{responses}", styles['Normal']))
            elements.append(Spacer(1, 20))
            # formatted_json=json.loads(results)
            elements.append(Paragraph(f"<b>Results: </b>", styles['Heading3']))
            ...
            response = HttpResponse(pdf, content_type='application/pdf')
            response['Content-Disposition'] = f'attachment; filename="Load_Test_{test.target.model_name}_report.pdf"'
            return response
        except Exception as e:
            print(str(e))
        # except Exception as e:
...
import pandas as pd 
class SecurityScanAnalytics(APIView):
    def get(self,request,test_id):
        try:
            scans=SecurityScanReport.objects.filter(test_id=test_id)
            d={
                "high":0,
                "low":0,
                "critical":0,
            ...
            for scan in scans:
                ...
                if result:
                    vulnerability_count=result.get("vulnerability_counts")
                    d['high']+=vulnerability_count.get('high')
                    d['low']+=vulnerability_count.get('low')
                    d['critical']+=vulnerability_count.get('critical')
                    d['medium']+=vulnerability_count.get('medium')
                    d['none']+=vulnerability_count.get('none')
                    d['risk_score']+=result.get('risk_score')
                else:
                    continue
...
class SecurityScanGraph(APIView):
    def get(self,request,test_id):
        ...
        for scan in scans:
            ...
            # print(result)
            vulnerability_count=result.get("vulnerability_counts")
            # print("vuln",vulnerability_count)
            # print(d['high'])
            d['high']+=vulnerability_count.get('high')
            # print(d['high'])
            d['low']+=vulnerability_count.get('low')
        ...
        plt.title(f"Severity Distribution Test {test.target.model_name}")
        plt.xticks(rotation=45,fontsize=10)
        plt.xlabel("Severity")
        plt.ylabel("Count")
        plt.ylim(0,max(d.values())+2)
        ax.legend(
